...
from handlers.admin._helpers import require_admin_callback, CTX_DELIVERY
...
DELIVERY_MESSAGE = 90  # ConversationHandler state: awaiting delivery message from admin
...
async def order_complete_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """ConversationHandler entry — admin tapped ✅ تکمیل شد on a PROCESSING order.

    Stores the order_id and asks the admin to type a delivery message (e.g. a
    license key or activation link), or to skip for a generic completion notice.
    Returns the DELIVERY_MESSAGE state.
    """
    query = update.callback_query
    if not await require_admin_callback(update):
...
async def order_delivery_get_message(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> int:
    """Receives the delivery message (or skip) from the admin, completes the order,
    ...
    order_id: Optional[int] = context.user_data.pop(CTX_DELIVERY, None)

    if order_id is None:
        await update.message.reply_text(
            "⚠️ خطا: اطلاعات سفارش یافت نشد. لطفاً دوباره تلاش کنید.",
            reply_markup=admin_main_menu_keyboard(),
        )
        return ConversationHandler.END

    is_skip = text == "⏭ رد کردن"
    delivery_msg: Optional[str] = None if is_skip else text
    ...
    if not order:
        await update.message.reply_text(
            _INVALID_ORDER_TRANSITION_TEXT,
            reply_markup=admin_main_menu_keyboard(),
        )
        return ConversationHandler.END

    await update.message.reply_text(
        f"✅ سفارش #{order_id} با موفقیت تکمیل شد.",
        reply_markup=admin_main_menu_keyboard(),
    )
    ...
    try:
        ...
        logger.warning(
            "Could not notify user %s about completed order %s.",
            order["user_id"],
            order_id,
        )
